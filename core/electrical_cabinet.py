...
def _notched_rect(w, h, nx, ny):
    """Прямоугольник с прямоугольными релиз-надрезами во всех 4 углах."""
    return [
        (0, ny), (nx, ny), (nx, 0), (w - nx, 0), (w - nx, ny), (w, ny),
        (w, h - ny), (w - nx, h - ny), (w - nx, h),
        (nx, h), (nx, h - ny), (0, h - ny),
    ]
...
def build_body(width=400, height=445, depth=150, thickness=1.2):
    w = int(round(width + 2 * depth + 14))    # 714
    h = int(round(height - 4))                # 441
    body = Part("Корпус", w, h, 1, thickness)
    body.bend_lines = [_bend("left", depth, "боковина"), _bend("right", depth, "боковина")]
    body.outline = _notched_rect(w, h, nx=int(round(w * 20 / 714)),
                                 ny=int(round(h * 18 / 441)))
    ...
    body.description = "Корпус шкафа: задняя стенка + боковины, гнётся из одного листа."
